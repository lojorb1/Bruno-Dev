...
const LoadingScreen: React.FC = () => {
  ...
  useEffect(() => {
    // Smoother step transition matching the progress speed
    const stepInterval = setInterval(() => {
      setStepIndex((prev) => (prev < LOADING_STEPS.length - 1 ? prev + 1 : prev));
    }, 350);

    // Optimized progress bar for production UX
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-slate-950 flex flex-col items-center justify-center p-6 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg aspect-square bg-blue-600/10 rounded-full animate-pulse-glow"></div>
      ...
      <div className="relative flex flex-col items-center max-w-sm w-full">
        ...
        <div className="h-16 text-center flex flex-col items-center justify-center w-full">
          <p className="text-white text-base md:text-lg font-bold tracking-tight animate-fade-in flex items-center gap-3">
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 opacity-20 flex items-center gap-2">
        ...
        <span className="text-[9px] font-medium text-slate-500">v2.2.0</span>
      </div>
    </div>
  );
};
