...
const Navbar: React.FC<NavbarProps> = ({ onHomeClick }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-slate-950/80 backdrop-blur-md py-4 border-b border-white/10' : 'bg-transparent py-6'}`}>
             ...
             <div className="flex items-center bg-white/5 border border-white/10 rounded-full px-1 py-1">
              {languages.map((l) => (
                <button
                  key={l.code}
                  onClick={() => setLang(l.code)}
                  className={`px-2 py-1 text-[10px] font-bold rounded-full transition-all ${lang === l.code ? 'bg-blue-600 text-white' : 'text-slate-400'}`}
                >
                  {l.label}
                </button>
              ))}
      ...
      <div className={`md:hidden absolute w-full bg-slate-900 border-b border-white/10 transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-96' : 'max-h-0'}`}>
        <div className="px-4 py-6 space-y-4">
          {navLinks.map((link) => (
            <a 
              ...
              className="block text-slate-300 text-lg py-2"
            >
              {link.name}
            </a>
          ))}
